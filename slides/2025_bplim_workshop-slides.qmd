...
## Link for the workshop materials

Worksop materials are available at:
```{r}
#| echo: false
code <- qrcode::qr_code("https://nareal.github.io/bplim_workshop_2025/")
plot(code)
```

[https://nareal.github.io/bplim_workshop_2025/](https://nareal.github.io/bplim_workshop_2025/){target="_blank"}
...
```{r}
tibble(
  approach = c("Naive R", "V3 R", "V3 R parallel", "V3 Rcpp parallel", "V3 Rcpp parallel (package)", "V3 Julia (parallelization in R)", "V3 Julia (parallelization in Julia)"),
  time = c(
  ...
  mutate(
    speedup = time[1] / time,
    `total time` = format(bench::as_bench_time(time))
  ) |> 
    select(-time) |> 
    gt() |> 
    opt_table_font(
...
```{r}
#| echo: false
code <- qrcode::qr_code("https://github.com/reisportela/cp2hdfe")
plot(code)
```
